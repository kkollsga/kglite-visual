...
| Agent-native | MCP at `/mcp` on the running server: sixteen tools over one shared, last-writer-wins view |
...
- **[Agents and MCP](agents.md)** — the flagship track. The launch contract,
  the JSON twin, the sixteen MCP tools, `window.__kglv`, and the rules about
