...
| Agent-native | MCP at `/mcp` on the running server: eighteen tools over one ordered shared view |
...
- **[Agents and MCP](agents.md)** — the flagship track. The launch contract,
  the JSON twin, the eighteen MCP tools, `window.__kglv`, and the rules about
